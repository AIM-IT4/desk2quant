...
export default async function handler(req, res) {
  ...
  if (!clientEmail || !privateKey) {
    return res.status(500).json({ ok: false, error: 'Drive credentials not configured in preview environment' });
  }
  ...
  try {
    ...
    if (!resp.ok) {
      return res.status(resp.status).json({ ok: false, status: resp.status, error: body?.error?.message || 'Drive lookup failed' });
    }
