...
async function addToBrevo(email) {
  ...
  if (!response.ok) throw new Error(`Brevo contact capture failed (${response.status})`);
}
...
async function sendResourceEmail(email, downloadUrl) {
  ...
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      ...
      htmlContent: `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#172033;line-height:1.6"><h2>Your FX Derivatives Quant Interview Workbook</h2><p>Thanks for joining the Desk2Quant free-resource list.</p><p>Your workbook contains <strong>50 FX derivatives interview problems</strong>, a formula reference sheet, and complete worked solutions.</p><p><a href="${downloadUrl}">Download the PDF</a></p><p style="font-size:13px;color:#667085">This private link expires in 7 days. You opted in to occasional Desk2Quant quant-finance resources and updates.</p></div>`,
      textContent: `Your Desk2Quant FX Derivatives Quant Interview Workbook: ${downloadUrl}`
    })
  });
  if (!response.ok) console.error('Free-resource email failed:', response.status);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await sendResourceEmail(email, `https://desk2quant.com${relativeUrl}`);
