...
const MODE_INSTRUCTIONS = {
  learn: 'Teach clearly from intuition to essential mathematics. For conceptual why/what questions, prioritize modelling intuition, trade-offs, and desk relevance. Keep the answer compact. Do not include code unless the user explicitly asks for code, implementation, simulation, calibration code, or debugging.',
  solve: 'Solve rigorously. State assumptions, derive step by step, check limiting cases or units where relevant, and clearly isolate the final result. Do not add code unless requested or the task itself is explicitly computational/programming-oriented.',
  ...
  project: 'Design one resume-worthy quant project with objective, mathematics, data, implementation milestones, validation, failure experiments, deliverables, and interview talking points. Include code architecture only when useful, not full implementation unless requested.'
};
...
function explicitlyRequestsCode(query = '') {
  const q = String(query).toLowerCase();
  return /\b(code|python|implement|implementation|script|notebook|debug|program|simulate|simulation|monte carlo code|calibration code)\b/.test(q);
}
...
export function buildAgentMessages(command, query, profile = {}) {
  ...
  const wantsCode = explicitlyRequestsCode(query);
  const system = [
    ...
    wantsCode
      ? 'The user explicitly requested implementation/code. Code may be included when it materially helps.'
      : 'The user did NOT explicitly request code. Do not output Python, pseudocode, fenced code blocks, implementation sketches, package imports, or code snippets. Answer with intuition, equations, modelling trade-offs, and quant-practitioner interpretation only.',
    'The response is displayed in a command-line terminal, not a browser. Write equations in terminal-native Unicode/plain text. Use symbols such as θ, σ, μ, Δ, ∂, ∫, √, ≤, ≥ and arrows directly. Write fractions as (numerator)/(denominator), powers as x^2 or exp(x), and named labels as σ_model or σ_mkt. Do NOT emit LaTeX delimiters or LaTeX commands such as \\( \\), \\[ \\], $$, \\frac, \\boxed, \\begin, \\theta, or \\sigma.',
    'For conceptual questions, default to roughly 5-12 concise paragraphs/bullets and at most 2-4 equations unless the user explicitly asks for a derivation.',
    'Be mathematically explicit and professionally concise. For calibrated no-arbitrage models, verify any numerical worked example before presenting it as correct.',
...
async function callModel(command, query, profile) {
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      temperature: command === 'solve' ? 0.2 : command === 'interview' ? 0.55 : 0.35,
      max_tokens: command === 'project' ? 2200 : wantsCode ? 2000 : command === 'learn' ? 1200 : 1600
    })
  });
  if (!response.ok) {
