...
export function buildAgentMessages(command, query, profile = {}) {
  ...
  const system = [
    ...
    'Use terminal-friendly Markdown/plain-text equations. Be mathematically explicit and professionally concise.',
...
function requireServerConfig(res) {
  if (!agentSecret()) {
    res.status(503).json({ error: 'Quant Agent is not configured: QUANT_AGENT_SECRET missing.' });
    return false;
  }
