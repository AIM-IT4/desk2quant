 ...
 * (`Bearer <secret>`), the `x-cron-secret` header, or the legacy `?secret=` query
 * param, so existing cron-job.org entries keep working while new ones can use the
 * header (query strings land in access logs and Referer headers).
 *
...
export function authorizeCronRequest(req) {
    ...
    const supplied = bearer || req.headers?.['x-cron-secret'] || req.query?.secret;
