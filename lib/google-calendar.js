...
export function buildCalendarEvent({ paymentId, customerEmail, customerName, sessionName, sessionDate, sessionTime, sessionDuration }) {
    ...
    return {
        // A deterministic ID makes webhook retries reuse the same event.
        id: `desk2quant${crypto.createHash('sha256').update(String(paymentId)).digest('hex').slice(0, 20)}`,
        ...
        // The service account creates the event; the customer receives the private
        // Meet link through Brevo. Attendees require Google Workspace delegation.
...
export async function createUniqueMeetEvent(booking, env = process.env) {
    ...
    if (response.status === 409) {
        // The same payment was retried after the event was already created.
        const existingResponse = await fetch(`${eventUrl}/${encodeURIComponent(eventRequest.id)}?conferenceDataVersion=1`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        if (!existingResponse.ok) throw new Error(`Google Calendar existing event lookup failed: ${await existingResponse.text()}`);
        event = await existingResponse.json();
    } else {
