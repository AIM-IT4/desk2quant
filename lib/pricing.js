 ...
 * @param {string} couponCode - optional, applied per-line-item like a single purchase
 */
async function getExpectedCartOrder(items, currency, couponCode) {
    if (!Array.isArray(items) || items.length === 0) {
        return { ok: false, error: 'Cart is empty' };
    }
    // Kept small enough that the compact "id:qty|id:qty" summary written into
    // the Razorpay order notes (256-char value limit per key) always fits.
    if (items.length > 15) {
        return { ok: false, error: 'Too many items in one checkout (max 15) — please split into two orders' };
    }
