...
async function validatePersonalizedCoupon(code, productId = null) {
    try {
        const resp = await fetch(`${SUPABASE_URL}/rest/v1/rpc/validate_recommendation_coupon_code`, {
            method: 'POST',
            headers: serviceHeaders({ 'Content-Type': 'application/json' }),
            body: JSON.stringify(productId ? { p_code: code, p_product_id: productId } : { p_code: code })
        });
...
async function resolveProductDiscountPercent(product, couponCode) {
    ...
    if (/^[A-Z]{2,40}(?:20|30)(?:[A-F0-9]{8})?$/.test(inputCodeUpper)) {
        const personalized = await validatePersonalizedCoupon(inputCodeUpper, product.id);
