...
import { createJitsiMeetingLink } from './jitsi.js';
...
export async function getMentorCatalog(){
    const [mentors,allSessions,slots]=await Promise.all([
        ...
        mentorDb('mentor_sessions?is_active=eq.true&select=id,mentor_id,title,description,prerequisites,outcomes,duration_minutes,price_paise&order=title'),
...
async function notifyMentorBooking(b){
    ...
    const body=confirmed?`${details}<p><a href="${escapeHtml(b.meet_link)}">Join your session</a></p>`:`${details}<p>Your payment was received, but this booking needs manual review. Desk2Quant will contact you to arrange a suitable time or refund.</p>`;
    ...
    const messages=[{type:'guest_customer',to:[{email:b.email,name:b.name}],html:body+`<p><a href="${escapeHtml(bookingManageUrl(b.email))}">Manage booking</a></p>`}];
    if(confirmed)messages.push({type:'guest_mentor',to:[{email:b.mentor_email,name:b.mentor_name}],html:body+`<p>Learner: ${escapeHtml(b.name)}</p><p>Preparation notes: ${escapeHtml(b.message||'None')}</p>`});
    messages.push({type:'guest_admin',to:[...new Set(adminEmails)].map(email=>({email})),html:body+`<p>Booking: ${escapeHtml(b.id)}</p><p>Learner: ${escapeHtml(b.name)} (${escapeHtml(b.email)})</p>`});
